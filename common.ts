...
import { readableStreamToString, KlarnaPaymentsError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new KlarnaPaymentsError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new KlarnaPaymentsError(e, e.response?.data)
                }
                if (innerError instanceof KlarnaPaymentsError) {
                    // Got "KlarnaPaymentsError" from the above try block
                    throw innerError;
                }
